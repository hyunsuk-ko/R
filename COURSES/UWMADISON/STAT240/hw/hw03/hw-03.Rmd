...
: According to the cyclical pattern of death, the expected death toll rises on first few months and falls on around middle of the year. The death rate increases in summer and decreases in winter.
...
1. Between April and May of 2020.
2. Between April and May of 2020.
3. The trend of excess deaths has been flattened out. 
...
: geom_col() draws bars at a specified height while geom_bar() draws bars based on the number of values within the level of the x-axis group.
...
RESPONSE
The one using the scale_y_log10() because it is easier to get a sense of the masses (without have to do a conversion)
...
:Exclude 'days' column from the dataset.
...
: Eliminate any row that has NA in value. / 700 rows -> 660 rows.
...
RESPONSE
- Seperates 'winter' new vector into year1 and year2.
- Changes the type of 'year1' column into numeric.
- Adds new 'year2' column which is 'year1' +1.
...
RESPONSE
- group_by function groups the data by 'winter'.
- n() counts the number of variables within the given group.
- sum() adds up the number of values.
- min() provides the minimum value of the variable.
- max() provides the maximum value of the variable.