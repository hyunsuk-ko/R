...
size4_v3 <- size4_v1 %>%
  mutate(p_binom = dbinom(boys, 4, p_4))
size4_v3
```
...
x<- rbinom(3,90,2/3)
x
tibble(pseq=seq(0,1,by=0.1), L=dbinom(x,90,pseq))
...
##
p_hat <- sum(x6*(0:6))/(6*sum(x6)) # boys / total
# (0*1096 + 1*6233 + ... + 6*1579) / 6 * (1096 + 6233 + ... + 1579)
p_hat

logl_1 <- sum(x6*dbinom(0:6,6,p_hat,log=TRUE)) # log likelihood of binomial model
logl_1
```
...
```{r est-bb}
bb_6 <- mlebb(x6)
# finds the maximum likelihood estimates of the parameters in the beta-binomial model
bb_6
```
