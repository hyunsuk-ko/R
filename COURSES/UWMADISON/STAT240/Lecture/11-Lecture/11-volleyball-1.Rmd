...
-> x and y are both on the nominator according to the formula, and doesn't matter
which one(x or y) comes first on the nominator.


2. Why do both variables have to be quantitative?  

-> As r has to be calculated, both variables have to be quantative.


3. How does changing the units of measurement change correlation?  

-> There is no 'unit' variable in the formula, so it has no effect.

4. Why doesn’t a tight fit to a horizontal line imply strong correlation?  

-> Horizontal line = no matter what x changes, it has no impact on y.
= no any sort of realtionship.
...
vb_regression_summary <- function(df,x,y)
{
  ...
  vb_sum <- df %>%
    group_by(Conference) %>%
    summarize(r = cor(!!x,!!y), ## !! = undo enquo
              b = get_slope(!!x,!!y))
