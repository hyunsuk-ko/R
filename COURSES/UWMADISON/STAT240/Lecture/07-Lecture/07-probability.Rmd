...
- `rbinom(n, size, prob)`  # Simulate binomial random variables
- `dbinom(x, size, prob)` # Binomial "density" calculations / gbinom(n,p) = Binomial "density" plot
- `pbinom(q, size, prob)` # Binomial distribution calculations
- `qbinom(p, size, prob)` # Binomial quantile calculations
...
## Binomial "density" calculations
 # gives heights of segments in below graph.
dbinom(0:n, n, p)
dbinom(0, n, p) 
dbinom(1, n, p) 
dbinom(2, n, p) 
...
prob <- 
  pnorm(b, mean=mu, sd=sigma, lower.tail=TRUE) -
  pnorm(a, mean=mu, sd=sigma, lower.tail=TRUE)
prob

ggplot(data.frame(x=c(mu-4*sigma, mu+4*sigma)), aes(x)) +
  stat_function(fun=dnorm, args= list(mean =mu, sd=sigma)) +
  stat_function(fun=dnorm, args= list(mean=mu, sd=sigma),
                xlim = c(a,b), geom="area", fill="magenta") +
