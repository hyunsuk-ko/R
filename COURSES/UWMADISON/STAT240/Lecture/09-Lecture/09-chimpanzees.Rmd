...
gbinom(90, 0.5, scale = TRUE, size = 2) +
  geom_vline(xintercept = 60, color = "red", linetype = "dashed", size = 2) +
  theme_bw()
```
...
pbinom(59,90,0.5, lower.tail = FALSE) * 2 # P(X > 59) = P(X >= 60)
pbinom(30,90,0.5,lower.tail = TRUE) * 2# P(X <= 30)

pbinom(60,90,0.5) - pbinom(29,90,0.5)
# P(X <= 60) - p(X <= 29) = P(30 <= X <= 60)
```
...
$$
H_0: p = 0.2 \\ # p = 1 / 5 cards (random setting)
H_a: p > 0.2
$$
...
est_c_diff  ## 95% CI for difference in proportions
...
df <- tibble( ## data frame with our simulated sample proportions and differences
  # rbinom(N,90,chimp_c$p_hat[2]),  chimp_c$p_hat[2] = probability
  p_hat_1 = rbinom(N, 90, chimp_c$p_hat[2]) / 90, # with partner
  p_hat_2 = rbinom(N, 30, chimp_c$p_hat[1]) / 30, # without partner
  diff = p_hat_1 - p_hat_2)
df

ci_rand <- df %>%
  summarize(se = sd(diff), ## standard deviation of the differences
            est = chimp_c$p_hat[2] - chimp_c$p_hat[1], ## estimated differences
...
$$
H_0: p_1 = p_2 \\ # implies p1-p2 = 0. z = 0.
H_a: p_1 \neq p_2
$$
...
- Estimate the test statistic under the assumption that the null hypothesis is true. ### IMPORTANT!!
    ...
    # pooled = don't differentiate btw with & w/o partner.
...
# under null hypothesis = TRUE, we don't have separate p_1 & p_2.
# So we just use the aggregated p, p_pool.
...
# = not able to reject the null hypothesis. The result is inconclusive.
...
df_e <- tibble(
  ...
  ## compare simulated difference to our observed difference
...
```{r LRT-calculations}
df_2 <- df_2 %>%
  mutate(p_0 = sum(prosocial) / sum(n)) %>%  # p_0 = identical probabilites
  mutate(log_L0 = dbinom(prosocial, n, p_0, log=TRUE), # null
         log_L1 = dbinom(prosocial, n, p_hat, log=TRUE)) # alternative
df_2
...
```{r chisq-p-value}
gchisq(6) +
  geom_chisq_fill(df=6, a=lrt$lrt, b=qchisq(0.999,6), fill="magenta") +
  theme_bw()
...
lrt_randomization <- function(df,N=100000)
{
  ...
  {
    df_rand <- df %>%
      mutate(prosocial = rbinom(m,n,p_0)) #p_0 = assuming null hypothesis is true.
