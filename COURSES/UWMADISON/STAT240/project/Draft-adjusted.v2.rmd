...
As we mentioned in the background, we want to reformat and join the datasets 
```{r data-cleaning-salaries-college-type, echo=FALSE, warning=FALSE, message=FALSE}
as_currency <- function(df) { # this function converts salaries to numbers from strings
  df %>%
  mutate_at(vars(contains("Salary")),str_remove_all,pattern="[$,]") %>%
  mutate_at(vars(contains("Salary")),as.numeric)
}

rename_lower <- function(x) { # this function standardizes names to R-compliant versions
  str_replace_all(x,"[ -]","_") %>%
  str_to_lower() %>%
...
For the numerical summaries, we made the first tibble of starting median salary and mid career median salary by the type of the school. The second tibble is similar but by the region of the school instead.   
...
type_and_region %>%
  ...
  xlab("Region of School") +
  ylab("Starting Median Salary") +
  ggtitle("Starting Median Salary vs Region of School", subtitle="By Type of School")
...
type_and_region %>%
  ...
  xlab("Region of School") +
  ylab("Mid Career Median Salary") +
  ggtitle("Mid Career Median Salary vs Region of School", subtitle="By Type of School")
...
region %>%
  ...
  ylab("Growth factor by mid-career") +
  ggtitle("Growth of salaries", subtitle = "By Region of School")
