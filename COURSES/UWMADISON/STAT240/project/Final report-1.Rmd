...
(a) Double-categorization can lead to distortion of our analysis.
```{r, echo=FALSE, warning=FALSE, message=FALSE}
type_party <- type %>%
  filter(type == 'Party') 
...
type %>%
  filter(name == 'University of Illinois at Urbana-Champaign (UIUC)') 

type %>%
  filter(name == 'Randolph-Macon College') 

double_type <- type_party %>%
  left_join(type_state, by="name")
