...
We found that there were statistically significant differences in the level of salary depends on the types of college and the level of salary varies depending on the location of school, but there are no significant differences .More specifically, The type of college makes a big difference in salary and the region of college, the location of school, makes no significant difference in salary, so type of college is more important than region. We also found that the salary increases faster over time for lower starting salaries for all types of college, with the exception of Ivy League colleges. Furthermore, we figured out that depends on the way that we grouped the types of schools, the results of the hypothesis test vary.
...
```{r data-cleaning-salaries-college-type, echo=FALSE, warning=FALSE}
...
```
We've set two functions. Firstly, we've set a reformatting function since the original data has salaries as strings like "$79,000.00" and this function converted those into numbers like 79000. Also, we've set a renaming function in order to make column names R-compliant. Above tables are the final versions of datasets and the last table is the combined data of salaries-by-college-type.csv and salaries-by-region.csv.
...
According to the both graphs, we expect to figure out the relationship between the type of school and starting, mid career salary with the region of school at the same time. It's important to note that engineering schools in every region tend to have the highest median of starting salary. In case of mid career median salary, the regions of California, Midwestern, and Western do not show any significant different from the cast of starting median salary, but in the region of Northeastern and Southern, the ivy league schools and liberal arts schools tend to have the highest mid career median salary, respectively.


### Linear models to show how does the initial salary related to increase in salary overtime by the types of school
```{r, echo=FALSE, warning=FALSE}
type %>%
...
According to the both plots, we can more easily conclude that people who are graduated from schools in California  and Northeastern tend to have the highest both starting median salary and mid career median salary. However, it's difficult to differentiate two regions so that we can say that the numerical approach is the better way for this analysis. Also, by virtue of the characteristic of the boxplot, we could figure out that there are some outliers in each region of schools. Furthermore, each region of schools has different salaries so that we can say that the location of school related to the level of individual's income after the graduation.
...
### Linear models to show how does the initial salary related to increase in salary overtime by the region of schools
```{r, echo=FALSE, warning=FALSE}
region %>%
...
### Provide broader interpretations of your analysis and describe how to interpret your results with respect to your questions of interest & Summarize your primary conclusions and the primary evidence that supports these conclusions.
 ...
 The primary curiosity of this project is how type and region of the school and the major affects salary. In terms of salary, we divided into two stages: starting and mid-career.
 
 In terms of type of school, the Ivy-League schools both had the highest mean salary on two stages followed by Engineering, Party, and State schools.
 
 In terms of region, Californian schools(especially engineering) had the highest starting mean salary.
...
(d) There are some overlapings in the data. For example, there are separate engineering schools like MIT, but also other type of universities like UW-Madison have engineering major. Thus, this overlappings in the data can affect the accuracy of an analysis on 'engineering' part.

(e) Ivy-League schools are entirely located at Northwestern region, which makes it difficult to make overall analysis across the region. What if we want to look out for 'State' schools across all regions? 
